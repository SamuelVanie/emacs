...
#+begin_src emacs-lisp
  ...
  ;; remove the symbols that shows line wrapping even if global-visual-line-mode
  ;; is not activated
  (setf (alist-get 'continuation
...
Starts the emacs server so that other programs could open files directly into emacs
