...
A modal editing binding tool that permits to avoid emacs' pinky
...
lsp mode is a company driven lsp client but with more features built-in than eglot.
...
can install emacs-lsp-booster globally using the nix command : =nix-env -iA nixpkgs.emacs-lsp-booster=
