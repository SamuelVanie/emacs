...
A tool that permits buffer ai completion in emacs
...
lsp-bridge, a blazingly fast and plug and play lsp client for emacs. 
...
Dape mode works with lsp-bridge, so mothing to that instead
...
There are some packages that I don't want to add to my emacs configuration, because they're disposable functionalities. Using nix, I can download such packages. The issue that arise, is how to load those package in my current emacs session ? I've written this function that automatically add their path to my load path, it automatically get the appropriate package version too.
...
Query request on any highlighted text in emacs
