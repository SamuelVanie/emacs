...
direnv permit to load environment on fly. You will need to have a .direnv file, you could generate it with =lorri init= command.
To use this plugin you must install lorri and direnv using : =nix-env -i lorri= and =nix-env -i direnv=.
