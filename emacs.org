...
If, you happen to use the xah-change-bracket-pairs function to quickly change brackets around a region it will result in the text selected, you can call the non interactive function (remove-overlays) to make it disappear.
...
Similar functionalities as vim-surround inside emacs.
...
Coding assistant directly inside emacs.
