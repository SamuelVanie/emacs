...
If the point is on a character in `xah-left-brackets', use `forward-sexp' to find the matching closing bracket and exclude both brackets.
If the point is on a character in `xah-right-brackets', use `backward-sexp' to find the matching opening bracket and exclude both brackets.
Otherwise, prompt the user for two characters to define the region."
  (interactive)

  ;; Define the bracket lists
  (unless (and (boundp 'xah-left-brackets) (boundp 'xah-right-brackets))
    (error "Please define `xah-left-brackets' and `xah-right-brackets' before using this function"))

  (let* ((current-char (char-after))  ; Get the character at the point
         (current-char-str (if current-char (char-to-string current-char) "")))

    ;; Check if the current character is in xah-left-brackets
    (if (and current-char (member current-char-str xah-left-brackets))
        (progn
      ...
      ;; Check if the current character is in xah-right-brackets
      (if (and current-char (member current-char-str xah-right-brackets))
          (progn
